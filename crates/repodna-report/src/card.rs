...
pub fn render_facts(facts: &Facts, options: CardOptions) -> String {
    ...
    let _ = write!(
        out,
        r#"<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" viewBox="0 0 {w} {h}" role="img" aria-labelledby="card-title card-desc" font-family="{font}"><title id="card-title">Project DNA card for {name}</title><desc id="card-desc">{desc}</desc>"#,
        w = WIDTH,
        h = HEIGHT,
        font = esc(FONT_STACK),
        name = esc(&facts.name),
